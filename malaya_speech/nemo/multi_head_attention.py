...
from malaya_speech.nemo.cast_utils import avoid_float16_autocast_context
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask, pos_emb=None, cache=None):
        ...
        if torch.is_autocast_enabled():
            query, key, value = query.to(
                torch.float32), key.to(
                torch.float32), value.to(
                torch.float32)
...
class RelPositionMultiHeadAttention(MultiHeadAttention):
    ...
    def rel_shift(self, x):
        ...
        # need to add a column of zeros on the left side of last dimension to
        # perform the relative shifting
...
class RelPositionMultiHeadAttentionLongformer(RelPositionMultiHeadAttention):
    ...
    def _get_global_attn_indices(self, is_index_global_attn: torch.Tensor) -> Tuple:
        ...
        is_local_index_no_global_attn_nonzero = (
            is_local_index_global_attn == 0).nonzero(
            as_tuple=True)
    ...
    def _compute_global_key_attn(
        ...
        # need to transpose since ONNX export only supports consecutive indexing:
        # https://pytorch.org/docs/stable/onnx.html#writes-sets
    ...
    def _compute_out_global_to_all(
        ...
        global_q_from_global = global_q_from_global.transpose(
            0, 1).reshape(batch_size * self.h, -1, self.d_k)
    ...
    def sliding_chunks_matmul_qk(
        ...
        diagonal_chunk_attn = self._skew(
            chunk_attn, direction=(
                0, 0, 0, 1), padding_value=padding_value)
    ...
    def sliding_chunks_matmul_pv(self, prob: torch.Tensor, v: torch.Tensor, w: int):
        ...
        chunk_v_stride = chunk_v_stride[0], w * \
            chunk_v_stride[1], chunk_v_stride[1], chunk_v_stride[2]
...
class RelPositionalEncoding(PositionalEncoding):
    ...
    def extend_pe(self, length, device):
        ...
        positions = torch.arange(
            length - 1, -length, -1, dtype=torch.float32, device=device).unsqueeze(1)
        self.create_pe(positions=positions)
