...
import torch
import random
from malaya_speech.torch_model.zipformer.encoder_interface import EncoderInterface
from scaling import (
    Balancer,
    BiasNorm,
    Dropout2,
        ...
        for i, module in enumerate(self.encoders):
            ...
            for layer in range(num_layers):
                ...
                cached_nonlin_attn = torch.zeros(
                    1, batch_size, downsample_left, nonlin_attn_head_dim).to(device)
                ...
                states += [cached_key, cached_nonlin_attn, cached_val1,
...
class Zipformer2EncoderLayer(nn.Module):
    ...
    src_key_padding_mask:  the mask for padding, of shape (batch_size, seq_len); True means
        ...
        src = src + (self_attn if self_attn_dropout_mask is None else self_attn *
        ...
        src = src + self.sequence_dropout(self.conv_module1(src,
                                                            chunk_size=chunk_size,
        ...
        if torch.jit.is_scripting() or torch.jit.is_tracing():
            ...
            conv_skip_rate = float(self.conv_skip_rate) if self.training else 0.0
        src = src + self.sequence_dropout(self.conv_module2(src,
                                                            chunk_size=chunk_size,
                                                            src_key_padding_mask=src_key_padding_mask),
                                          conv_skip_rate)
    ...
    def forward(
        ...
        if use_pos_scores:
            ...
            if torch.jit.is_tracing():
                ...
                pos_scores = pos_scores.as_strided((num_heads,
                                                    batch_size,
                                                    seq_len,
                                                    seq_len),
                                                   (pos_scores.stride(0),
