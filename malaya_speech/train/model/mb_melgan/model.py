...
from ..melgan.model import Generator as generator
import numpy as np
...
class PQMF(tf.keras.layers.Layer):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.taps = taps
        self.analysis_filter = tf.convert_to_tensor(
            analysis_filter.astype(np.float32)
        )
        self.synthesis_filter = tf.convert_to_tensor(
            synthesis_filter.astype(np.float32)
        )
        self.updown_filter = tf.convert_to_tensor(
            updown_filter.astype(np.float32)
        )
