  ...
  """
    ...
    """
        if query_layer is not None and not isinstance(
            query_layer, layers_base.Layer
        ):
            raise TypeError(
                'query_layer is not a Layer: %s' % type(query_layer).__name__
            )
        if memory_layer is not None and not isinstance(
            memory_layer, layers_base.Layer
        ):
            raise TypeError(
        ...
        if score_mask_value is None:
            score_mask_value = dtypes.as_dtype(
                self._memory_layer.dtype
            ).as_numpy_dtype(-np.inf)
        self._probability_fn = lambda score, prev: (  # pylint:disable=g-long-lambda
            probability_fn(
                _maybe_mask_score(
                    score, memory_sequence_length, score_mask_value
                ),
                prev,
            )
        )
  ...
  """
    ...
    if scale:
        ...
        g = variable_scope.get_variable(
            'attention_g', dtype=dtype, initializer=1.0
        )
        score = g * score
    return score
    ...
    if normalize:
        ...
        g = variable_scope.get_variable(
            ...
            initializer=init_ops.constant_initializer(
                math.sqrt(1.0 / num_units), dtype=dtype
            ),
        )
...
class ZhaopengLocationLayer(layers_base.Layer):
  ...
  """
    ...
    def __call__(self, prev_attention, query):
        ...
        fertility = math_ops.sigmoid(
            math_ops.reduce_sum(math_ops.multiply(self.vbeta, query))
        )
...
def safe_cumprod(x, *args, **kwargs):
  ...
  """
    with ops.name_scope(None, 'SafeCumprod', [x]):
        ...
        return math_ops.exp(
            math_ops.cumsum(
                math_ops.log(clip_ops.clip_by_value(x, tiny, 1)),
                *args,
                **kwargs
            )
        )
    ...
    if mode == 'recursive':
        ...
        attention = p_choose_i * array_ops.transpose(
            functional_ops.scan(
                ...
                # iterations
                lambda x, yz: array_ops.reshape(
                    yz[0] * x + yz[1], (batch_size,)
                ),
        ...
        attention = (
            ...
            * math_ops.cumsum(
                ...
                clip_ops.clip_by_value(cumprod_1mp_choose_i, 1e-10, 1.0),
                axis=1,
            )
        )
    elif mode == 'hard':
    ...
    **NOTE** If you are using the `BeamSearchDecoder` with a cell wrapped in
    ...
    """
        ...
        if cell_input_fn is None:
            def cell_input_fn(inputs, attention): return array_ops.concat(
                [inputs, attention], -1
            )
        else:
    ...
    **NOTE** Please see the initializer documentation for details of how
    ...
    """
        ...
        if self._output_attention == True:
            ...
            return (
                array_ops.concat((cell_output, attention), axis=-1),
                next_state,
            )
        else:
            raise ValueError(
