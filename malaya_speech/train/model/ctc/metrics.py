...
def ctc_sequence_accuracy(
    logits, logits_lengths, targets, targets_lengths, beam_width=1, merge_repeated=True
):
    logits = tf.transpose(logits, [1, 0, 2])
    Y_seq_len = targets_lengths
    max_Y_seq_len = tf.reduce_max(Y_seq_len)
    ...
    preds = preds[:, : max_Y_seq_len]
    preds = tf.pad(preds, [[0, 0], [0, max_Y_seq_len - tf.shape(preds)[1]]])
    masks = tf.sequence_mask(
        Y_seq_len, max_Y_seq_len, dtype=tf.float32
    )
