...
class SpeechRecognitionProblem(problem.Problem):
    ...
    def hparams(self, defaults, model_hparams):
        ...
        add_if_absent(p, 'audio_sample_rate', 22050)
    ...
    def preprocess_example(self, example, mode, hparams):
        ...
        if p.audio_preproc_in_bottom:
            ...
            variance = tf.reduce_mean(
                tf.squared_difference(mel_fbanks, mean),
                keepdims = True,
                axis = 1,
            )
            ...
            example['inputs'] = tf.concat(
                [
                    ...
                    tf.zeros(
                        (p.num_zeropad_frames, fbank_size[2], fbank_size[3])
                    ),
                ],
                0,
            )
        ...
        return super(SpeechRecognitionProblem, self).preprocess_example(
