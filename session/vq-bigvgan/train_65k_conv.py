...
class VQMelConv(nn.Module):
    def __init__(
        ...
        self.in_proj = nn.Linear(mel_dim, latent_dim)

        blocks = []
        for i in range(num_layers):
            dilation = dilation_base ** i
            blocks.append(ConvBlock(latent_dim, kernel_size=kernel_size, dilation=dilation))
        self.blocks = nn.Sequential(*blocks)
        self.out_proj = nn.Linear(latent_dim, mel_dim)
    ...
    def forward(self, mel, attention_mask):
        ...
        x = self.blocks(x)
        mel = self.out_proj(x)
        return mel.permute(0, 2, 1), indices
    ...
    class Dataset(torch.utils.data.Dataset):
        ...
        def __getitem__(self, idx):
            try:
                ...
                segment_length = h.sampling_rate * 1
