...
def main():
    ...
    accelerator.init_trackers(
        ...
        init_kwargs={
            "wandb": {
                "name": data_args.wandb_run_name}} if data_args.wandb_run_name else {},
    )
    ...
    if dataset_was_precomputed:
        ...
        if training_args.do_train:
            ...
            raw_datasets["train"] = raw_datasets["train"].rename_column('audio_filename', 'audio')
        ...
        if training_args.do_eval:
            ...
            raw_datasets["eval"] = raw_datasets["eval"].rename_column('audio_filename', 'audio')
    ...
    if not dataset_was_precomputed:
        ...
        # see:
        # https://huggingface.co/docs/accelerate/main/en/package_reference/accelerator#accelerate.Accelerator.prepare
        ...
        def apply_audio_decoder(batch):
            ...
            # if `pad_to_max_length`, the maximum corresponding audio length of the
            # current batch is max_duration*sampling_rate
        ...
        for split in vectorized_datasets:
                ...
                for i, batch in enumerate(
                    ...
                    if accelerator.is_main_process:
                        ...
                        if ((cur_step + 1) % data_args.save_codec_steps == 0) or (
                            ...
                            save_codec_checkpoint(
                                os.path.join(
                                    data_args.temporary_save_to_disk,
                                    split),
                                tmp_labels,
                                cur_step)
                            all_generated_labels = []
            ...
            with accelerator.local_main_process_first():
                tmp_labels = load_all_codec_checkpoints(os.path.join(
                    data_args.temporary_save_to_disk, split)).select(range(len(vectorized_datasets[split])))
                logger.info(
    ...
    if data_args.save_to_disk is not None and not dataset_was_precomputed:
        if accelerator.is_main_process:
            vectorized_datasets.save_to_disk(
                data_args.save_to_disk, num_proc=min(
                    data_args.preprocessing_num_workers, len(
                        vectorized_datasets["eval"]) - 1), )
        accelerator.wait_for_everyone()
    ...
    if checkpoint is not None:
        ...
        if training_args.max_steps < 0:
            # we know exactly the number of steps per epoch, so can skip through the
            ...
            with accelerator.autocast(autocast_handler=autocast_kwargs):
                if training_args.parallel_mode.value != "distributed":
                    encoder_outputs = model.text_encoder(
                        input_ids=batch.get("input_ids"), attention_mask=batch.get(
                            "attention_mask", None))
                else:
                    encoder_outputs = model.module.text_encoder(input_ids=batch.get(
                        "input_ids"), attention_mask=batch.get("attention_mask", None))
    ...
    def generate_step(batch, accelerator):
        ...
        if training_args.torch_compile:
            # if the model is compiled, we use the original model bc compile is not
            # compatible with .generate
            eval_model = model._orig_mod

        # since we've might have loaded the weights in fp32, we have to autocast to ensure FA2 weights are in half-precision.
        # with
        # accelerator.autocast(autocast_handler=AutocastKwargs(enabled=(attn_implementation=="flash_attention_2"))):
    ...
    for epoch in range(epochs_trained, num_epochs):
        ...
        sampler = None
        if training_args.group_by_length:
            sampler = LengthGroupedSampler(train_batch_size,
            ...
            if accelerator.sync_gradients:
                ...
                if training_args.do_eval and (cur_step %
                    ...
                    for batch in tqdm(
                        ...
                        eval_metric = {
                            key: val.unsqueeze(0) if val.ndim == 0 else val for (
                                key, val) in eval_metric.items()}
                    ...
                    eval_metrics = {key: torch.mean(torch.cat([d[key] for d in eval_metrics])).to(
                        "cpu") for key in eval_metrics[0]}
