...
def get_dataset(batch_size = 40, shuffle_size = 128, prefetch_size = 128):
    def get():
...
iterator = get_dataset()().make_one_shot_iterator().get_next()
inputs = tf.expand_dims(iterator['inputs'], -1)
targets = tf.expand_dims(iterator['targets'], -1)
...
sess, fetches = pix2pix.model.get_session(get_generator, inputs, targets)
epochs = 500_000
checkpoint = 5000
output_dir = 'out-pix2pix-resnet34'

writer = tf.summary.FileWriter(f'./{output_dir}')
