...
class Model:
    def __init__(self, X, Y, size = 4):
        ...
        self.outputs = unet.Model3D(
            D_X, dropout = 0.0, training = True, cout = size, kernel_size = 5
        ).logits
        ...
        for i in range(size):
            self.loss.append(
                tf.reduce_mean(
                    tf.abs(
                        self.outputs[:, :, :, :, i : i + 1] - self.stft[i][1]
                    )
                )
            )
...
save_directory = 'speaker-split-unet3d-combined'
