...
def get_alignment(f):
    f = f"tacotron2-female-singlish-alignment/{f.split('/')[-1]}"
    if os.path.exists(f):
        return np.load(f)
    else:
        return None
...
pitch_stat = np.load('../speech-bahasa/female-singlish-stats/stats_pitch.npy')
...
train.run_training(
    ...
    model_fn=model_fn,
    model_dir='fastpitch-female-singlish',
    num_gpus=1,
    log_step=1,
