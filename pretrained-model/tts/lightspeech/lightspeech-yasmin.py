...
files = glob('../speech-bahasa/output-yasmin/mels/*.npy')
files.extend(glob('../speech-bahasa/output-yasmin-parliament/mels/*.npy'))
...
def get_alignment(f):
    f = f"tacotron2-yasmin-alignment/{f.replace('../speech-bahasa/', '').replace('/', '-')}"
    if os.path.exists(f):
        return np.load(f)
    else:
        return None
...
def model_fn(features, labels, mode, params):
    ...
    mel_before = mel_before[:, : max_length]
    mel_before = tf.pad(mel_before, [[0, 0], [0, max_length - tf.shape(mel_before)[1]], [0, 0]])

    mel_after = mel_after[:, : max_length]
    mel_after = tf.pad(mel_after, [[0, 0], [0, max_length - tf.shape(mel_after)[1]], [0, 0]])
