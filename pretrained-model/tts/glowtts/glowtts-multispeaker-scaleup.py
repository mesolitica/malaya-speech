...
files = glob('/home/husein/speech-bahasa/*/mels/*.npy')
files = [f for f in files if '-vits' not in f]
...
def generate(files):
    ...
    while True:
        ...
        mel = np.load(f)
        mel = mel * stats[speaker][1] + stats[speaker][0]
...
train.run_training(
    ...
    model_fn=model_fn,
    model_dir='glowtts-multispeaker-scaleup',
    num_gpus=1,
    log_step=1,
