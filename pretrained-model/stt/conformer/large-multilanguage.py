...
len_vocab = min([l.vocab_size for l in langs])
...
def model_fn(features, labels, mode, params):
    ...
    transducer_model = transducer.rnn_multilanguage.Model(
        conformer_model, language_size=len(langs) + 1,
    ...
    logits, lang_logits = transducer_model([v, c, targets_length + 1], training=True)
    ...
    cost_lang = transducer.loss.rnnt_loss(
        logits=lang_logits,
        labels=features['lang'],
        label_length=targets_length,
        logit_length=logits_len,
    )
    cost = tf.reduce_mean(cost)
    cost_lang = tf.reduce_mean(cost_lang)
    loss = cost + cost_lang

    tf.identity(loss, 'train_loss')
    tf.identity(cost, 'cost')
    tf.identity(cost_lang, 'cost_lang')

    tf.summary.scalar('cost', cost)
    tf.summary.scalar('cost_lang', cost_lang)

    # variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES)
    # variables = [v for v in variables if 'transducer_prediction' in v.name]
    # init_checkpoint = 'transducer-rnn-large/model-rename.ckpt'
...
train_hooks = [tf.train.LoggingTensorHook(['train_loss', 'cost', 'cost_lang'], every_n_iter=1)]
...
train.run_training(
    ...
    model_fn=model_fn,
    model_dir='asr-large-multilanguage-conformer-transducer-v2',
    num_gpus=1,
    log_step=1,
