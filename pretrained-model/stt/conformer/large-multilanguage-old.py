...
len_vocab = max([l.vocab_size for l in langs])
        ...
        for i in range(len(audios)):
            try:
                ...
                t = malaya_speech.subword.encode(
                    langs[lang], cleaned_texts[i], add_blank=False
                )
    ...
    def get():
        ...
        dataset = dataset.padded_batch(
            ...
            padding_values={
                ...
                'lang': tf.constant(0, dtype=tf.int32),
            },
        )
        return dataset
...
def model_fn(features, labels, mode, params):
    ...
    print(len(langs), len_vocab)
    transducer_model = transducer.rnn_multilanguage.Model(
        conformer_model, language_size=len(langs),
        vocabulary_size=len_vocab, training_mode=True, **decoder_config
    )
    ...
    enc = transducer_model.encoder(v, training=True)
    lang_ = tf.tile(tf.expand_dims(lang, -1), (1, tf.shape(enc)[1]))
    logits, lang_logits = transducer_model([v, c, targets_length + 1, lang_], training=True)
    logits_len = features['inputs_length'][:, 0] // conformer_model.conv_subsampling.time_reduction_factor
    ...
    masks = tf.sequence_mask(logits_len, tf.shape(enc)[1], dtype=tf.float32)
    lang_loss = tf.contrib.seq2seq.sequence_loss(logits=lang_logits,
                                                 targets=lang_,
                                                 weights=masks)

    loss = mean_error + lang_loss

    tf.identity(loss, 'train_loss')
    tf.identity(mean_error, 'mean_error')
    tf.identity(lang_loss, 'lang_loss')

    tf.summary.scalar('mean_error', mean_error)
    tf.summary.scalar('lang_loss', lang_loss)
...
train.run_training(
    ...
    model_fn=model_fn,
    model_dir='asr-large-multilanguage-conformer-transducer',
    num_gpus=1,
    log_step=1,
