...
class MalayaDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        x = self.data['X'][idx]
        y = self.data['Y'][idx]
        y = ''.join([c for c in y if c in self.CTC_VOCAB])
        ...
        if random.random() > 0.95 and self.AUGMENTATION:
            try:
...
if __name__ == '__main__':
    ...
    model_directory = f"{model_name.replace('/', '-')}-ms-{precision}"
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, num_workers=3, collate_fn=batch)
