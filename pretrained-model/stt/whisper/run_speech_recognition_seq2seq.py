...
class MalayaDataset(torch.utils.data.Dataset):
    def __init__(self, file, audio, feature_extractor, tokenizer, do_lower_case=True):
    ...
    def __getitem__(self, idx):
        x = self.data['X'][idx]
        y = self.data['Y'][idx]
        r = self.audio.decode_example(audio.encode_example(x))

        inputs = self.feature_extractor(r['array'], sampling_rate=r['sampling_rate'])
        r[model_input_name] = inputs[model_input_name][0]
        r['input_length'] = len(r[model_input_name])

        input_str = y.lower() if do_lower_case else y
        r["labels"] = self.tokenizer(input_str).input_ids
