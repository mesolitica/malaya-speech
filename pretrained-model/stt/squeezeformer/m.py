...
parameters = {
    ...
    'lr_policy_params': {
        'warmup_steps': 40000,
        'max_lr': (0.05 / config['encoder_dmodel']),
    },
}
...
def model_fn(features, labels, mode, params):
    ...
    length_v = features['inputs_length'][:, 0]
    z = tf.zeros((tf.shape(features['targets'])[0], 1), dtype=tf.int32)
    c = tf.concat([z, features['targets']], axis=1)

    logits = transducer_model([v, length_v, c, targets_length + 1], training=True)

    cost = transducer.loss.rnnt_loss(
        logits=logits,
        labels=features['targets'],
        label_length=targets_length,
        logit_length=length_v // model.encoder.conv_subsampling.time_reduction_factor,
    )
...
train.run_training(
    ...
    model_fn=model_fn,
    model_dir='asr-m-squeezeformer-transducer',
    num_gpus=1,
    log_step=1,
