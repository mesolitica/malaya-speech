...
def get_optimizer(learning_rate, hparams):
  ...
  """
    return {
        'rmsprop': tf.RMSPropOptimizer(
            learning_rate,
            decay = 0.95,
            momentum = hparams.momentum,
            epsilon = 1e-4,
        ),
        'adam': tf.AdamOptimizer(
            learning_rate, beta1 = 0.9, beta2 = 0.999, epsilon = 1e-8
        ),
        'adagrad': tf.AdagradOptimizer(
            learning_rate, initial_accumulator_value = 1.0
        ),
        'mom': tf.MomentumOptimizer(learning_rate, momentum = hparams.momentum),
        'sgd': tf.GradientDescentOptimizer(learning_rate),
  ...
  """
    ...
    if re_im:
        ...
        if log_mag:
            mag = (
                librosa.power_to_db(
                    mag ** 2, amin = 1e-13, top_db = 120.0, ref = np.max
                )
                / 120.0
            ) + 1
        else:
...
def griffin_lim(mag, phase_angle, n_fft, hop, num_iters):
  ...
  """
    fft_config = dict(
        n_fft = n_fft, win_length = n_fft, hop_length = hop, center = True
    )
...
def ispecgram(
  ...
  """
    ...
    ifft_config = dict(
        win_length = n_fft, hop_length = hop_length, center = True
    )
...
def form_image_grid(input_tensor, grid_shape, image_shape, num_channels):
  ...
  """
    ...
    if len(input_tensor.get_shape()) == 2:
        ...
        if (
            int(input_tensor.get_shape()[1]) != image_shape[0]
            or int(input_tensor.get_shape()[2]) != image_shape[1]
            or int(input_tensor.get_shape()[3]) != num_channels
        ):
            raise ValueError(
                'Image shape and number of channels incompatible with '
                'input tensor.'
    ...
    if hparams.raw_audio:
        ...
        if image:
            ...
            tf.summary.image(
                'Mag/%s' % name,
                form_image_grid(
                    spec[:b, :, :, :1], [rows, columns], [n_freq, n_time], 1
                ),
            )
            if phase:
                tf.summary.image(
...
def softmax_summaries(loss, logits, one_hot_labels, name = 'softmax'):
  ...
  """
    ...
    tf.summary.scalar(
        name + '_precision@1', tf.reduce_mean(tf.to_float(in_top_1))
    )
    in_top_5 = tf.nn.in_top_k(logits, one_hot_labels, 5)
    tf.summary.scalar(
        name + '_precision@5', tf.reduce_mean(tf.to_float(in_top_5))
    )
...
def frequency_weighted_cost_mask(
    peak = 10.0, hz_flat = 1000, sr = 16000, n_fft = 512
):
  ...
  """
    n = int(n_fft / 2)
    cutoff = np.where(
        librosa.core.fft_frequencies(sr = sr, n_fft = n_fft) >= hz_flat
    )[0][0]
...
# Neural Nets
# ---------------------------------------------------
def pitch_embeddings(
    batch, timesteps = 1, n_pitches = 128, dim_embedding = 128, reuse = False
):
...
def causal_linear(
    x, n_inputs, n_outputs, name, filter_length, rate, batch_size
):
  ...
  """
    ...
    # create queue
    q_1 = tf.FIFOQueue(
        rate, dtypes = tf.float32, shapes = (batch_size, 1, n_inputs)
    )
    q_2 = tf.FIFOQueue(
        rate, dtypes = tf.float32, shapes = (batch_size, 1, n_inputs)
    )
